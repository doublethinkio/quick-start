...
const counterSelector = Selector('[data-testid="count"]')
...
test('should navigate to Counter with click on the "to Counter" link', async (t) => {
  await t.click('[data-testid=container] > a').expect(getCounterText()).eql('0')
})
...
test('should back to home if back button clicked', async (t) => {
  await t
    .click('[data-testid="backButton"] > a')
    .expect(Selector('[data-testid="container"]').visible)
    .ok()
})
